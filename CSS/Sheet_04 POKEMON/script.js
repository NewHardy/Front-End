...
function createCard(foto, id, name, type) {
  ...
  pokeCard.appendChild(cardTop, cardBottom);
  cardTop.appendChild(image, ID);
  cardBottom.appendChild(pokeName, pokeTypes);
...
function getPokemonsUrls(arrayUrls) {
  fetch("https://pokeapi.co/api/v2/pokemon")
    .then((response) => response.json())
    .then((result) => {
